...
const char* speed_model_to_string(SpeedModel model) {
  switch (model) {
    ...
    case SPEED_AUTO: return "Auto";
    case SPEED_LAUNCH: return "Launch";
    case SPEED_ANTI_SLIP: return "Anti-Slip";
    case SPEED_VALET: return "Valet";
    case SPEED_SL: return "SL";
...
void HikeITBLEComponent::attempt_connection() {
  ...
  // Let BLE client handle the connection
  // The connection will be confirmed in gattc_event_handler
}
...
uint8_t HikeITBLEComponent::calculate_checksum(const uint8_t *data, size_t len) {
  uint32_t sum = 0;
  for (size_t i = 0; i < len; i++) {
    sum += data[i];
  }
...
std::vector<uint8_t> HikeITBLEComponent::build_message(uint8_t type, const uint8_t *content) {
...
void HikeITBLEComponent::send_verify_command() {
  ...
  uint8_t content[10] = {0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void HikeITBLEComponent::send_disconnect_command() {
  ...
  uint8_t content[10] = {0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  auto cmd = this->build_message(0x09, content);
  this->send_command(cmd);
...
void HikeITBLEComponent::send_screen_command() {
  ...
  uint8_t content[10] = {0x24, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  auto cmd = this->build_message(0x08, content);
  this->send_command(cmd);
}

void HikeITBLEComponent::send_speed_model_command(SpeedModel model, uint8_t at_flag) {
  if (!this->has_cached_state_) {
    ESP_LOGW(TAG, "No cached state, cannot send speed model command");
    return;
  }
  
  ESP_LOGI(TAG, "Sending speed model command: %s (AT=%d)", speed_model_to_string(model), at_flag);
...
void HikeITBLEComponent::send_step_command(uint8_t step, SpeedModel model) {
  ...
  ESP_LOGI(TAG, "Sending step command: %d for %s", step, speed_model_to_string(model));
...
void HikeITBLEComponent::send_safe_mode_command(const std::string &password, bool enable) {
  ...
  uint8_t content[10] = {
    pwd_low, pwd_high, pwd_low, pwd_high,
...
void HikeITBLEComponent::handle_notification(const uint8_t *data, uint16_t length) {
...
void HikeITBLEComponent::process_message(const uint8_t *data) {
  ...
  ESP_LOGI(TAG, "Parsed message - Type: 0x%02X, Count: %d, ID: %08X",
           msg.type, msg.count, msg.device_id);
  ...
  if (msg.type == 0x02) {
    ...
    if (this->speed_select_ != nullptr) {
      this->speed_select_->publish_state(speed_model_to_string(msg.speed_model));
    }
    ...
    ESP_LOGI(TAG, "  Version: %.1f, Locked: %s, AT: %d",
             msg.version, msg.is_safe_model ? "YES" : "NO", msg.at_flag);
  }
...
bool HikeITBLEComponent::parse_message(const uint8_t *data, size_t len, ParsedMessage &msg) {
  ...
  if (calc_checksum != msg.checksum) {
    ESP_LOGW(TAG, "Checksum mismatch: expected %02X, got %02X", calc_checksum, msg.checksum);
    return false;
  }
